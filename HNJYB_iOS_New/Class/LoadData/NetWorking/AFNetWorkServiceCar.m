    ...
    if(NSOrderedSame == compareRet)
    {
        [manager POST:url parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
              ...
              failure:^(AFHTTPRequestOperation *operation, NSError *error)
         {
             ...
             NSLog(@"error ~ %@",error.description);
             if (error.code == -1005) {
                 [self requestWithServiceIP3:serviceIP ServiceName:serviceName params:params httpMethod:httpMethod resultIsDictionary:resultIsDictionary completeBlock:block2 timeOut:time];
             }
             if(block2 != nil)
             {
                 self.count = 0;
