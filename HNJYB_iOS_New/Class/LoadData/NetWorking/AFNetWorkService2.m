...
static AFNetWorkService2 *service2;
...
+ (AFNetWorkService2 *)getAFNetWorkService2
{
    ...
    dispatch_once(&once, ^{
        if (!service2)
        {
            service2 = [[AFNetWorkService2 alloc]init];
        }
    });
    return service2;
}
...
-(void)requestWithServiceIP2:(NSString *)serviceIP ServiceName:(NSString *)serviceName params:(NSMutableDictionary *)params httpMethod:(NSString *)httpMethod resultIsDictionary:(BOOL)resultIsDictionary completeBlock:(RequestCompelete2)block2
{
    ...
    if([self getNetState] == 0)
    {
        if(_showNotice)
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"没有网络连接" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alert show];
        }
        if(block2 != nil)
        {
        ...
        [manager POST:url parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject)
         {
             ...
             if(responseObject != nil)
             {
                 ...
                 else
                 {
                     ...
                     else if([@"-4" isEqualToString:restate])
                     {
                         //发送通知
...
//                         }
//                     }
                     else if([@"-93" isEqualToString:restate] || [@"-94" isEqualToString:restate] || [@"-95" isEqualToString:restate] || [@"-97" isEqualToString:restate] || [@"-98" isEqualToString:restate])
                     {
                         //防止无限循环
                         self.count++;
                         if(self.count > 3)
